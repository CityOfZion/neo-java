...
public class StateMachine extends InteropService {
	...
	//
	// private Dictionary<UInt160, UInt160> contracts_created = new
	// Dictionary<UInt160, UInt160>();
	//
	...
	//
	public StateMachine(final Block persisting_block, final Map<UInt160, AccountState> accounts,
			final Map<UInt256, AssetState> assets, final Map<UInt160, ContractState> contracts,
			final Map<StorageKey, StorageItem> storages) {
	...
	// (AssetType)(byte)engine.EvaluationStack.Pop().GetBigInteger();
	// if (!Enum.IsDefined(typeof(AssetType), asset_type) || asset_type ==
	// AssetType.CreditFlag || asset_type == AssetType.DutyFlag || asset_type ==
	// AssetType.GoverningToken || asset_type == AssetType.UtilityToken)
	// return false;
	...
	// Fixed8 amount = new
	// Fixed8((long)engine.EvaluationStack.Pop().GetBigInteger());
	...
	// if (amount != -Fixed8.Satoshi && amount.GetData() % (long)Math.Pow(10, 8 -
	// precision) != 0)
	// return false;
	// ECPoint owner =
	// ECPoint.DecodePoint(engine.EvaluationStack.Pop().GetByteArray(),
	// ECCurve.Secp256r1);
	...
	// engine.EvaluationStack.Pop().GetByteArray().Select(p =>
	// (ContractParameterType)p).ToArray();
	// if (parameter_list.Length > 252) return false;
	// ContractParameterType return_type =
	// (ContractParameterType)(byte)engine.EvaluationStack.Pop().GetBigInteger();
	// ContractPropertyState contract_properties =
	// (ContractPropertyState)(byte)engine.EvaluationStack.Pop().GetBigInteger();
	...
	// string email =
	// Encoding.UTF8.GetString(engine.EvaluationStack.Pop().GetByteArray());
	// if (engine.EvaluationStack.Peek().GetByteArray().Length > 65536) return
	// false;
	// string description =
	// Encoding.UTF8.GetString(engine.EvaluationStack.Pop().GetByteArray());
	...
	// {
	// foreach (var pair in
	// storages.Find(engine.CurrentContext.ScriptHash).ToArray())
	// {
	...
	// return false;
	// if (!created.Equals(new UInt160(engine.CurrentContext.ScriptHash))) return
	// false;
...
// protected override DataCache<StorageKey, StorageItem> Storages => storages;
//
// public StateMachine(Block persisting_block, DataCache<UInt160, AccountState>
// accounts, DataCache<UInt256, AssetState> assets, DataCache<UInt160,
// ContractState> contracts, DataCache<StorageKey, StorageItem> storages)
// {
// this.persisting_block = persisting_block;
